...
func ScaleWithOffset(bc Barcode, width, height int, offset int) (Barcode, error) {
	switch bc.Metadata().Dimensions {
	case 1:
		return scale1DCodeWithOffset(bc, width, height, offset)
	case 2:
		return scale2DCodeWithOffset(bc, width, height, offset)
	}
...
func scale2DCodeWithOffset(bc Barcode, width, height, offset int) (Barcode, error) {
	orgBounds := bc.Bounds()
	orgWidth := orgBounds.Max.X - orgBounds.Min.X
	orgHeight := orgBounds.Max.Y - orgBounds.Min.Y
...
func scale1DCodeWithOffset(bc Barcode, width, height, offset int) (Barcode, error) {
	orgBounds := bc.Bounds()
	orgWidth := orgBounds.Max.X - orgBounds.Min.X
